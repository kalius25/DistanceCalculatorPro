"""VietBanDo web route extraction engine."""
...
from app.parsers.vietbando_parser import VietBanDoParser
...
class VietBanDoEngine(BaseEngine):
    """Navigate to VietBanDo and extract the total route distance."""
    ...
    def __init__(
        ...
        if action_timeout <= 0:
            raise ValueError("VietBanDo action timeout must be greater than zero.")
    ...
    def navigate(
        ...
        """Navigate to the complete VietBanDo directions URL."""
    ...
    def find_routes(
        ...
        """Navigate, wait for the total-distance result, and parse it."""
        url = VietBanDoUrlBuilder.build(request)
        self._navigate(page, request, url)

        try:
            distance = VietBanDoLocator.route_distance(page)
            distance.wait_for(
                state="visible",
                timeout=self._action_timeout,
            )
            routes = VietBanDoParser.parse(page, self._diagnostics)
            if not routes:
                raise EngineException(
                    "VietBanDo returned no parseable route distance.",
            ...
            raise EngineException(
                "VietBanDo route results timed out.",
            ...
            raise EngineException(
                "VietBanDo result extraction failed.",
                error_code=ErrorCode.ENGINE_ERROR,
                cause=error,
                context=self._context(request, url),
            ) from error
    ...
    def _navigate(
        ...
        try:
            ...
            self._diagnostics.trace_browser(
                logger,
                "VIETBANDO_NAVIGATION_COMPLETED",
                url=url,
            )
        except PlaywrightTimeoutError as error:
            raise EngineException(
                "VietBanDo request timed out.",
                error_code=ErrorCode.ENGINE_ERROR,
                cause=error,
                context=context,
            ) from error
