...
def test_constructor_rejects_non_positive_timeout() -> None:
    with pytest.raises(
        ...
        OpenStreetMapEngine(0)


def test_navigate_opens_complete_osm_url(
    route_request: RouteRequest,
) -> None:
    page = MagicMock()
    diagnostics = MagicMock()
    engine = OpenStreetMapEngine(15_000, diagnostics)
...
def test_navigate_wraps_playwright_timeout(
    ...
    with pytest.raises(
        EngineException,
        match="OpenStreetMap request timed out",
    ) as error:
...
def test_navigate_wraps_playwright_error(
    ...
    with pytest.raises(
        EngineException,
        match="OpenStreetMap browser operation failed",
    ):
        engine.navigate(page, route_request)
...
def test_default_diagnostics_manager_is_created() -> None:
    ...
    with patch(
        "app.engines.openstreetmap_engine.DiagnosticsManager",
        return_value=diagnostics,
    ):
        engine = OpenStreetMapEngine(15_000)
...
def test_find_routes_rejects_empty_parse(
    ...
    with (
        ...
        with pytest.raises(
            EngineException,
            match="OpenStreetMap returned no parseable routes",
        ):
...
def test_find_routes_wraps_result_timeout(
    ...
    with patch(
        ...
        with pytest.raises(
            EngineException,
            match="OpenStreetMap route results timed out",
        ):
            engine.find_routes(page, route_request)
...
def test_find_routes_wraps_result_playwright_error(
    ...
    with patch(
        ...
        with pytest.raises(
            EngineException,
            match="OpenStreetMap result extraction failed",
        ):
            engine.find_routes(page, route_request)
...
def test_context_contains_route_identity(
    ...
    context = OpenStreetMapEngine._context(
        route_request,
        "https://example.test",
    )
